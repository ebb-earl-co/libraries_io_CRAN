...
def main(argv=None):
    ...
    r_projects_on_cran_query: str = \
        """MATCH (:Language {name: 'R'})
        <-[:IS_WRITTEN_IN]-(p:Project {merged_contributors: %d})
        <-[:HOSTS]-(:Platform {name: 'CRAN'})
    ...
    # from SQLite
    print("Querying Neo4j for nodes representing R projects on CRAN\n",
          file=sys.stderr)
    projects_cursor: Cursor = execute_cypher_match_statement(
        g, r_projects_on_cran_query % int(merged_contributors)
    )
    ...
    with connect(DB) as conn:
        ...
        for batch in batches:
            ...
            for pnode_cnode in batch:
                ...
                print("MERGEing contributors to Neo4j for R project "
