...
def populate_sqlite_project_names(conn, table, names):
    ...
    Args:
        ...
        table (str): table in the DB connected to via `conn`
        names (iterable): iterable of strings
    Raises:
        (SystemExit): upon sqlite3 error durin insert statements
    Returns:
        (int): rowcount of how many records inserted
    """
...
def return_parser():
    ...
    parser.add_argument('DB', type=str,
                        help='The sqlite DB containing project_names and '
                        'information about queries to the Libraries.io API.')
    parser.add_argument('table', type=str,
                        help='The name of the table in the sqlite DB passed '
                        'to the `DB` argument')
...
def main():
    ...
    # Query Neo4j for all R projects
    ...
    cypher_query = ("MATCH (:Platform{name:'CRAN'})-[:HOSTS]->"
                    "(p:Project)-[:IS_WRITTEN_IN]->"
                    "(:Language{name:'R'}) return p.name order by p.name")
    print(f"Executing Cypher query:\n{cypher_query}\n", file=sys.stderr)

    result = execute_cypher_query(driver, cypher_query)
    project_names = [record.get('p.name') for record in result.records()]
