...
*N.b. the environment variable `APIKEY` must be set in order to send requests
to the Libraries.io API.*
...
the error that occurred during the request(s) is returned in JSON format to
STDOUT.
"""
import itertools
import json
...
URL = "https://libraries.io/api/CRAN/%s/contributors"
...
def build_get_request(url, get_api_key=True, per_page=100, page=None):
    ...
    def get_api_key_():
        ...
        if api_key is None:
            print("'APIKEY' is not among environment variables!",
                  file=sys.stderr)
            sys.exit(1)
...
def parse_request_response_content(r):
    """ Given a `requests.Response` object, call `r`.raise_for_status(). Return
    namedtuple with the first field either: None if an exception arose; or
    r.content and second field either: the exception that arose; or None
    otherwise.
    Args:
